...
function responseInterceptors(http: HttpRequestAbstract) {
  ...
  http.interceptors.response.use((response: HttpResponse) => {
    ...
    if (data.code === 50004) {
      uni.reLaunch({
        url: `/pages/register/index?info=${JSON.stringify(data.data)}`,
      })
    ...
    // 信息不完整，需要完善用户信息
      ...
      uni.redirectTo({
        url: '/pages/personal/edit?type=home',
      })
      // return refreshToken(http, config)
    }
    // thirdSessionKey不匹配,登录状态失效，跳转错误页面，重新登录
    ...
    if (custom?.toast !== false && data.code !== 50004 && data.code !== 50005) {
      // console.log(`!!!!!!!!!!!!!!!!!!!!!!!!!`)
      uni.$u.toast(data.errMsg || data.message)
    }
